...
from ..config.lstm import TrainerConfig


class LSTMCnnVAETrainer:
    def __init__(self, model: nn.Module, dataloader, config: TrainerConfig, val_dataloader=None):
        self.model = model
    ...
    def _get_beta(self, step):
        if step >= self.config.beta_warmup_iters:
            return self.config.beta_end

        progress = step / self.config.beta_warmup_iters
    ...
    def train_step(self, step):
        ...
        with autocast(
            ...
            lambda_fb = 0.5  # minimalne nats per dim (tune: 0.1 - 2.0)
            kl_per_dim = -0.5 * (1 + logvar - mu.pow(2) - torch.exp(logvar))
            # shape: (batch, latent_dim)
            kl_loss = torch.mean(torch.clamp(kl_per_dim, min=lambda_fb))
    ...
    def _save_checkpoint(self, step):
        path = f"{self.config.checkpoint_dir}/model_step_{step}.pt"
    ...
    def _save_history(self):
        path = f"{self.config.checkpoint_dir}/history.json"
